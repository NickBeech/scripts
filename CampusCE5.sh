...
# Log file with current
LOG_FILE="$LOG_DIR/delete_old_files_$($DATE +%b%d%Y).log"
...
for DIR in "$TARGET_DIR" "$SOURCE_DIR"; do
    if [ ! -d "$DIR" ]; then
        echo "$($DATE) - ERROR: '$DIR' does not exist." >> "$LOG_FILE"
        exit 1
    fi
done
...
chmod -R 666 "$TARGET_DIR" >> "$LOG_FILE" 2>&1


echo "$($DATE) - Checking files older than 14 days..." >> "$LOG_FILE"
find "$SOURCE_DIR" -maxdepth 1 -type f -mtime +14 | while read -r src_file; do
    file_name=$(basename "$src_file")
    if [ -f "$TARGET_DIR/$file_name" ]; then
        rm -f "$src_file" "$TARGET_DIR/$file_name"
        echo "$DATE) - Deleted '$file_name' from both directories." >> "$LOG_FILE"
    fi
